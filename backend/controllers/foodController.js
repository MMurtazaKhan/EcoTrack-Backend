...
const carbonFootprintDataLowercase = Object.keys(carbonFootprintData).reduce(
  (acc, key) => {
    acc[key.toLowerCase()] = carbonFootprintData[key];
    return acc;
  },
  {}
);
...
const getCarbonFootprint = (req, res) => {
  ...
  // Use the lowercase version of the food item to check in the modified carbonFootprintData
  const foodLowercase = food.toLowerCase();
  if (!(foodLowercase in carbonFootprintDataLowercase)) {
    return res.status(404).json({ error: "Food item not found" });
  }

  // Calculate the carbon footprint based on the given kg
  const carbonFootprintPerKg = carbonFootprintDataLowercase[foodLowercase];
